...
// Mobile Navbar - No Framer Motion
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-colors duration-300 ${
        scrolled ? "bg-black/90 border-b border-gray-800/50" : "bg-transparent"
      }`}
    >
      ...
      {isMenuOpen && (
        <div className="bg-black/95 border-t border-gray-800/50">
          <div className="px-4 py-6 space-y-2">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block px-4 py-3 text-gray-300 hover:text-cyan-400 rounded-xl"
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 mx-4 mt-4 bg-cyan-500 text-white px-6 py-3 rounded-xl font-medium"
            >
...
// Desktop Navbar with animations
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <motion.div
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Link href="/" className="text-3xl font-bold text-gradient-animated">
              Vexio
            </Link>
          </motion.div>
